    ...
    fun generateClass1To3SubjectCsv(
        ...
        FileOutputStream(file).use { fos ->
            ...
            OutputStreamWriter(fos, StandardCharsets.UTF_8).use { writer ->
                ...
                val n1Title = if (isCls3) "நானே செய்வேன் - 1 [20 மதிப்பெண்]" else "நானே செய்வேன் - 1 [25 மதிப்பெண்]"
                val n2Title = if (isCls3) "நானே செய்வேன் - 2 [20 மதிப்பெண்]" else "நானே செய்வேன் - 2 [25 மதிப்பெண்]"
                val thTitle = if (isCls3) "திறனறி மதிப்பீடு [60 மதிப்பெண்]" else "திறனறி மதிப்பீடு [50 மதிப்பெண்]"
                val oral12 = if (isCls3) "வாய்மொழி [5]" else "வாய்மொழி [10]"
                val act12 = "செயல்திட்டம் / செயல்பாடு [10]"
                val wri12 = "எழுத்துவழி [5]"
                val thWriStr = if (isCls3) "எழுத்துவழி [50]" else "எழுத்துவழி [40]"

                writer.appendLine(
                    "\"வரிசை எண்\",\"சேர்க்கை எண்\",\"மாணவர் பெயர்\"," +
                    "\"$n1Title\",\"\",\"\",\"" +
                    "\"$n2Title\",\"\",\"\",\"" +
                    "\"$thTitle\",\"\",\"மொத்தம் [100]\",\"விழுக்காடு\""
                )
                writer.appendLine(
                    "\"\",\"\",\"\",\"$oral12\",\"$act12\",\"$wri12\"," +
                    "\"$oral12\",\"$act12\",\"$wri12\"," +
                    "\"வாய்மொழி [10]\",\"$thWriStr\",\"[100]\",\"%\""
                )
